...
**상태:** Migration·삭제·쓰기 Backend/Frontend · RC-1·RC-2 Release Candidate **PASS**
**D-2 완료일:** 2026-07-22  
...
**비범위 (잔여):** Bulk Delete, Drag & Drop, Category CRUD, History UI, TMDB, 인증, 자동화 Browser E2E
...
- Collection POST/PATCH Frontend (C-2 완료)
...
- Bulk Delete, Drag & Drop, Vitest/RTL E2E, Category CRUD, History UI, TMDB, 인증

### I-3 (2026-07-23)

- Collection 상세 「제거」= PATCH `collection_id:null` (Hard Delete 아님)
- 마지막 Item 연결 해제 시에도 빈 Collection **유지** (DELETE 자동 Collection 삭제와 구분)
- Backend **196 passed** · 격리 DB `picknext_i3_write_regression` Smoke **33/33** · Seed 비파괴

### RC-1 (2026-07-23)

- **API·격리 DB Release Candidate 검증: 통과** (`picknext_write_rc` Smoke **40/40**)
  - 빠른 연결 해제 ≠ Hard Delete
  - 마지막 Item unlink → Collection GET 200 · `item_count=0`
  - 마지막 Item DELETE → Collection GET 404 · DELETE 204 body 길이 0
- 당시 브라우저 수동 QA 대기 → **RC-2에서 완료**

### RC-2 (2026-07-23)

- 격리 Backend `:8001` · Frontend `:5175` 브라우저 수동 QA **PASS**
- **RC-2-A PASS:** Items checkbox Warning · Item Form Inline Validation
- 브라우저에서 마지막 unlink 유지 / 마지막 DELETE Collection 삭제 정책 구분 재확인
- **기본 쓰기 Release Candidate 최종 판정: PASS**

### I-2 (2026-07-23)

- Item 생성·수정 Frontend 연동 완료
- Item PATCH 연결 해제·이동은 빈 Collection 유지 (DELETE 자동 Collection 삭제와 구분)
- Collection 상세 행 「제거」는 I-3에서 연동

### I-1 (2026-07-22)

- `POST`/`PATCH /api/v1/items` Backend 구현 완료
- PATCH 연결 해제·이동은 빈 Collection을 유지 (자동 삭제 없음)
- 기존 Item DELETE Transaction·History 전체 삭제·마지막 Collection 자동 삭제 **변화 없음** (회귀 테스트 통과)
...
- Collection 생성·수정 Frontend Dialog·API 연동 완료
- 기존 삭제 정책·Transaction **변화 없음**
