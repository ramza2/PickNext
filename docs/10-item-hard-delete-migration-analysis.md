...
**비범위 (잔여):** Collection 직접 DELETE(D-6), Item/Collection POST·PATCH, Frontend 쓰기
...
- Collection 직접 DELETE / Item≥1 → 409: **미구현 (D-6)**
...
`DELETE /api/v1/collections/{id}` — Item≥1 → 409, 빈 Collection → 204
