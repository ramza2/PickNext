...
> **상태:** **Backend 읽기·삭제·Collection POST/PATCH 구현 완료 · Frontend 목록·상세·삭제·Item 빠른 연결 해제 연동 완료**
> **범위:** Collection 목록·상세 읽기 API 요청·응답·정책, Query 권장 구조, 테스트, Backend 구현, Frontend 목록·상세 연동  
> **비범위:** Bulk Delete, Drag & Drop, Index 추가
...
I-1: 2026-07-22 — `POST`/`PATCH /api/v1/items` Backend 구현. Item 연결·이동·해제(PATCH)로 빈 Collection 유지. Collection `updated_at` Touch 없음. Collection 직접 DELETE 409 계약 유지.
I-2: 2026-07-23 — Item 수정 Dialog에서 Collection 연결·이동·해제 가능.
I-3: 2026-07-23 — Collection 상세 Item 「제거」= `PATCH collection_id:null` (Item 유지·빈 Collection 유지). 페이지 보정 재사용. 빠른 상태 버튼 PATCH 연동.
RC-1: 2026-07-23 — **API·격리 DB** `picknext_write_rc` Write Smoke **40/40** (CRUD·unlink·last unlink 빈 Collection 유지·DELETE 자동 삭제 대조·204 body 0). **브라우저 수동 QA 대기**. 판정 **PASS WITH NOTES**.
...
- 읽기 계약(목록·상세 Query·Response)은 변경 없음.
- 쓰기: `POST` 201 / `PATCH` 200 — 응답은 동일 `CollectionResponse`. `DELETE`는 D-6 계약 유지.
- 소속 Item:
...
- `DELETE /api/v1/items/{id}` (D-3~D-5): Hard Delete + 포함 RecommendationHistory 전체 삭제
- 마지막 Item DELETE 시 소속 Collection도 같은 Transaction에서 Hard Delete → 상세/목록에서 Collection 404
- PATCH 연결 해제·이동으로 빈 Collection이 된 경우는 **유지** (I-1 Backend · I-2 Form · I-3 Collection 상세 「제거」)
...
- Collection 상세 「제거」(I-3)는 Item DELETE가 아니며, 마지막 Item 연결 해제 후에도 Collection 상세 Empty State를 유지한다.
